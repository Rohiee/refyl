...
class Hyori_Socials_Widget extends Goal_Widget {
    public function __construct() {
        parent::__construct(
            'goal_socials_widget',
            esc_html__('Goal Socials', 'hyori'),
            array( 'description' => esc_html__( 'Socials for website.', 'hyori' ), )
        );
    ...
    public function scripts() {
        wp_enqueue_script( 'hyori-script-admin-js', get_template_directory_uri().'/js/admin/admin.js', array( 'jquery' ), GOAL_FRAMEWORK_VERSION, true );
    }
    ...
    <div class="goal_socials">
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'socials' )); ?>"><?php esc_html_e('Select socials:', 'hyori'); ?></label>
            <br>
        <?php
...
if ( function_exists('goal_framework_reg_widget') ) {
    goal_framework_reg_widget('Hyori_Socials_Widget');
}